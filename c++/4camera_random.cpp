...
// see line 519 for using camera capture or using local images
...
int gl_stop = 0;
const char* onnxPath = "onnx_new.onnx";
const char* enginePath = "trt_new.trt";
...
void cameraHolo::initialize() {
    ...
    checkCudaErrors(cudaMalloc(&posxDevice, H * W * C * sizeof(float)));
    checkCudaErrors(cudaMalloc(&posyDevice, H * W * C * sizeof(float)));
    ...
    fillCuda(posxDevice, cutW, cutH, 1);
    fillCuda(posyDevice, cutW, cutH, 1);
...
void cameraHolo::printNetworkInfo(INetworkDefinition* network) {
    ...
    inDims = network->getInput(1)->getDimensions();// assume 4
    inputDim = inDims.nbDims;
    cout << "input2 num: " << inputNum << std::endl;
    cout << "input2 dim: " << inputDim << std::endl;
    cout << "input2 size: " << inDims.d[0] << "," << inDims.d[1] << ","
        << inDims.d[2] << "," << inDims.d[3] << std::endl;

    inDims = network->getInput(2)->getDimensions();// assume 4
    inputDim = inDims.nbDims;
    cout << "input3 num: " << inputNum << std::endl;
    cout << "input3 dim: " << inputDim << std::endl;
    cout << "input3 size: " << inDims.d[0] << "," << inDims.d[1] << ","
        << inDims.d[2] << "," << inDims.d[3] << std::endl;
...
void cameraHolo::printEngineInfo()
{
    ...
    inputName = engine->getIOTensorName(1);
    inputShape = engine->getTensorShape(inputName);
    cout << "inputname: " << inputName << std::endl;;
    inputDim = inputShape.nbDims;
    assert(inputDim == 4);
    cout << "input dim: " << inputDim << std::endl;
    cout << "input size: " << inputShape.d[0] << "," << inputShape.d[1]
        << "," << inputShape.d[2] << "," << inputShape.d[3] << std::endl;

    inputName = engine->getIOTensorName(2);
    inputShape = engine->getTensorShape(inputName);
    cout << "inputname: " << inputName << std::endl;;
    inputDim = inputShape.nbDims;
    assert(inputDim == 4);
    ...
    auto outputName = engine->getIOTensorName(3);
...
void cameraHolo::setupInference() {
    ...
    assert(status != false);
    status = context->setTensorAddress("onnx::Concat_1", posxDevice);
    assert(status != false);
    status = context->setTensorAddress("onnx::Concat_2", posyDevice);
    assert(status != false);
